 ...
 */
export const DEFAULT_CONFIG: ClaudeProxyConfig = {
  version: 1,
  routing: {
    agent_routing: true,
    model_routing: false,
 ...
 */
export function validateClaudeProxyConfig(config: ClaudeProxyConfig): ConfigValidationResult {
  const errors: string[] = [];
  ...
  if (config.routing.agent_routing && config.routing.model_routing) {
    errors.push(
      "agent_routing and model_routing are mutually exclusive. Only one can be enabled at a time."
    );
  }
  ...
  if (!config.routing.agent_routing && !config.routing.model_routing) {
    warnings.push(
      "Both routing modes are disabled. Requests will use fallback model directly."
    );
  }
 ...
 */
export function loadClaudeProxyConfig(options: { silent?: boolean } = {}): ClaudeProxyConfig | null {
  ...
  try {
    ...
    const config: ClaudeProxyConfig = {
      version: (rawConfig.version as number) ?? DEFAULT_CONFIG.version,
      routing: {
        agent_routing:
          (rawConfig.routing as Record<string, boolean> | undefined)?.agent_routing ??
          DEFAULT_CONFIG.routing.agent_routing,
        model_routing:
          (rawConfig.routing as Record<string, boolean> | undefined)?.model_routing ??
          DEFAULT_CONFIG.routing.model_routing,
      },
      fallback: {
        model:
          (rawConfig.fallback as Record<string, unknown> | undefined)?.model as string ??
          DEFAULT_CONFIG.fallback.model,
        provider:
          (rawConfig.fallback as Record<string, unknown> | undefined)?.provider as string ??
          DEFAULT_CONFIG.fallback.provider,
      },
      logging: {
        level:
          ((rawConfig.logging as Record<string, unknown> | undefined)?.level as ClaudeProxyConfig["logging"]["level"]) ??
          DEFAULT_CONFIG.logging.level,
        format:
          ((rawConfig.logging as Record<string, unknown> | undefined)?.format as ClaudeProxyConfig["logging"]["format"]) ??
    ...
    if (!silent) {
      console.warn(
        `[claude-proxy] Failed to load config: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    return null;
  }
 ...
 */
export function isAgentRoutingEnabled(): boolean {
 ...
 */
export function getRoutingMode(): "agent" | "model" | "passthrough" {
  const config = loadClaudeProxyConfig({ silent: true });
 ...
 */
export function getFallbackConfig(): ClaudeProxyConfig["fallback"] {
  const config = loadClaudeProxyConfig({ silent: true });
